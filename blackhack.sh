...
function checkUpdate(){
    GIT=$(curl --silent https://github.com/v7y3/blackhack/blob/main/blackhack.sh | grep 'VERSION=' | cut -d">" -f2 | cut -d"<" -f1 | cut -d"=" -f 2)
...
function installUpdate(){
    ...
    git clone https://github.com/v7y3/blackhack &>/dev/null
    chmod +x blackhack/blackhack.sh && mv blackhack/templates/* templates &>/dev/null
    mv blackhack/blackhack.sh . &>/dev/null
    mv blackhack/get_ip.php . &>/dev/null
    mv blackhack/info.php . &>/dev/null
    if [ "$(echo $?)" == "0" ]; then
...
if [ "$(echo $1)" == "--update" ]; then
    clear; banner
    echo -e "${BBlue}[+]${Color_Off} ${BWhite}blackhack Versión $VERSION${Color_Off}"
    echo -e "${BBlue}[+]${Color_Off} ${BWhite}Verificando actualización de blackhack${Color_Off}"
    checkUpdate
